...
import {isESports, isEsportsById} from '@core/utils';
...
export default React.memo(() => {
  ...
  const menuItems = [
    ...
    {content: isESports() ? '玩法规则' : '体育竞猜规则', path: '/rule'},
  ...
  const [selectedIndex, setSelectedIndex] = useState(isESports() ? 1 : 0);
  ...
  useEffect(() => {
    if (selectedIndex < 2 && selectedIndex !== -1) {
      setSelectedIndex((isEsportsById(sportId)) ? 1 : 0);
    }
