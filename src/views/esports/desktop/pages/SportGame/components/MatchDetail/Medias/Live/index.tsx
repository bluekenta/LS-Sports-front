...
export default React.memo(({videos, auto = true}: IProps) => {
  ...
  return (
    <div
      className={classnames(style.wrapper, 'my-live')}
      style={{backgroundImage: `url(${currentVideo && currentVideo.screenshot})`}}
      onMouseEnter={() => setMenuShow(true)}
      onMouseLeave={() => setMenuShow(false)}>
      {/* { !currentVideo && <div className="empty">当前比赛没有视频源</div> } */}
      {currentVideo && (
        <iframe
          src={`https://video.dmjsopw.com/?auto=${auto}&liveUrl=${isFlv ? currentVideo.srouceUrl : currentVideo.m3u8}`}
