...
import LeagueItem from '@this/pages/SportGame/components/LeagueList/LeagueItem';
...
import {eSportsAllCategory} from '@constants/enum/sport/sportsCategory';
...
export default React.memo(() => {
  ...
  useEventEmitter<TMitt['syncFavoriteLeagueId']>({
    mittName: 'syncFavoriteLeagueId',
    on: ({leagueId}) => {
  ...
  React.useEffect(() => {
    ...
    if (!intersection.length) setAllOpen(false);
    else if (intersection.length === leaugeIds.length) setAllOpen(true);
  ...
  React.useEffect(() => {
    ...
    const fn = () => {
      ...
      // }
      dispatch(
        ACTIONS.SPORT.getMatchListByMatchIds({
          data: null,
          cb: (res: {data: TMatchData}) => {
            ...
            const leagueList: TLeague[] = [];
            const map: any = {};
            matchList.leagues.forEach((item) => {
              if (map[item.leagueId]) return;
              map[item.leagueId] = true;
              leagueList.push(item);
            });
            ...
            setData({matches: mergedMatches, leagues: leagueList});
            emit({leagues: leagueList, matches: mergedMatches});
            // 第一次进入切换第一场比赛为详情赛事
            if (!initalTarget.current) {
              const leaugeIds = leagueList.map((league) => `fav#${league.leagueId}`);
              ...
              const firstLeague = _.chain(leagueList)
                .groupBy('sportId')
                .map((item, key) => item)
                .flatten()
                .first()
                .value();
              const firstMatchId = matchList.matches.find(
                (item) => item.leagueId === firstLeague.leagueId && !item.isChampion,
              )?.matchId;
              if (firstMatchId) {
  ...
  const handleGoBack = React.useCallback(() => {
    ...
    db.matches
      ...
      .then((matches) => {
        const match = _.find(
          matches,
          (item) => !item.isChampion && item.isLive === (league.state === EMatchTypes.IN_PLAY),
        );
        if (match) {
  ...
  const toggleBySportId = (sportId: number) => {
    if (toggleData[sportId].open) {
      ...
      dispatch(
        ACTIONS.SPORT.updateUserSettings(
          'collapseLeagueIds',
          _.uniq(collapseLeagueIds.concat(toggleData[sportId].lids)),
        ),
      );
    }
  };
  ...
  return (
    <>
      <div className={styles['favorite-detail']}>
        ...
        <div className={`${leagueListStyles.wrapper} favorite-detail-list`}>
          ...
          {data?.leagues?.length > 0 &&
            _.map(
              _.groupBy(
                data.leagues.filter((i) => (currentLeagueId === 0 ? i : i.leagueId === currentLeagueId)),
                'sportId',
              ),
              (leagueGroup, key) => (
                <div key={key}>
                  <div
                    className={classnames('play-type-wrapper', {'is-closed': !toggleData[Number(key)].open})}
                    onClick={() => toggleBySportId(Number(key))}>
                    <DpIcon type='goback2' className={classnames({'is-closed': !toggleData[Number(key)].open})} />
                    <span>{_.find(eSportsAllCategory, {sportId: Number(key)})?.name}</span>
                    <span className='count'>
                      ( {_.filter(data.matches.filter((i) => i.sportId === Number(key)))?.length} )
                    </span>
                  </div>
