...
export default React.memo(({match, matchWithState, gameBettingType, gameNames}: IProps) => {
  ...
  const [computedDivHeight, setComputedDivHeight] = React.useState<string | number>(
    match.sportId === 1 && playMethod.length ? 158 + 'px' : 120 + 'px',
  );
  ...
  const bodanRender = React.useCallback(
    (subMarkets: (undefined | PlayType)[]) => {
      return subMarkets.map((market, mdx) => {
        return market?.mks[0]?.ops ? (
          <div className='play-content' key={mdx}>
              ...
              <div className='other'>
                {market?.mks[0]?.ops
                  ...
                  .map((bil, bidx) =>
                    bil ? (
                      <OddItem
                        bil={{...bil, name: '其他', sportId: match.sportId}}
                        kc={market.code}
                        key={bil?.id || bidx}
                      />
                    ) : (
          ...
          <div className={classnames('oddItemEmpty', {oddItemEmpty_bg: !mdx})}>
            {new Array(9).fill('-').map((item, indx) => (
    ...
    return (
      <div className='subClassifyContent'>
                    ...
                    return (
                      <>
                        ...
                        <div className='game-name'>
                          {market?.code === 'FT_CS'
                            ? '全场平局'
                            : market?.mks[0]?.ops[0]?.teams.home.name && '半场平局'}
                        </div>
        ...
        <div className='teamName'>
          <div
            className={classnames('team-name', {'text-red text-bold': match.teams.home.isHandicap})}
            onClick={() => openZoom(match.matchId)}>
            <Tooltip title={match.teams.home.name} placement='top'>
              {match.teams.home.name}
              {match.isLive && typeName[0]?.name === '角球' ? (
                <span className='ml-10'>{`${match.score.homeCorner}`}</span>
              ) : (
                `-${typeName[0]?.name}`
              )}
            </Tooltip>
          ...
          <div
            className={classnames('team-name', {'text-red text-bold': match.teams.away.isHandicap})}
            onClick={() => openZoom(match.matchId)}>
            <Tooltip title={match.teams.away.name} placement='top'>
              {match.teams.away.name}
              {match.isLive && typeName[0]?.name === '角球' ? (
                <span className='ml-10'>{`${match.score.awayCorner}`}</span>
              ) : (
                `-${typeName[0]?.name}`
              )}
            </Tooltip>
        ...
        {!subBtnActives.bodan ? (
          <BettingOdds markets={subMarkets} sportId={match.sportId} gameList={gameListSub} />
        ) : (
          bodanRender(subMarkets)
        )}
      </div>
    );
  ...
  return (
    <div
      className={classnames(styles.match_item, {
        active: currentMatchId === match.matchId,
        noSubPlayMethod: !playMethod.length || match.sportId !== ESportType.FOOTBALL,
      })}
      ref={ref}
      style={{minHeight: !playMethod.length || match.sportId !== ESportType.FOOTBALL ? undefined : computedDivHeight}}>
      {inView && (
        <>
          ...
          {match.sportId === 1 && playMethod.length > 0 && (
            <div className={classnames('subClassify', {bottomLine: unfold})}>
              <div className='arrow' onClick={handleArrow}>
                {' '}
                <div className={classnames(`open-icon ${unfold ? 'open' : ''}`)} />
              </div>
              {playMethod.map((item) => (
                <div
                  key={item.type}
                  className={classnames('btn', {active: subBtnActives[item.type as toggleBtnType]})}
                  onClick={() => handleToggleBtn(item.type as toggleBtnType)}>
                  {item.name}
                </div>
              ))}
            </div>
