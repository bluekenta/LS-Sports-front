        ...
        <div className='team-odds'>
          ...
          {market?.mks?.[0] && (
            <div className='team-odds-list'>
              {market.mks[0].ops.map((op: OddType) => (
                <SingleOddItem code={market.code} op={{...op, sportId}} key={op.id} />
              ))}
            </div>
          )}
        </div>
...
export const SingleOddItem = React.memo(({op, code}: {op: OddType; code: string}) => {
  ...
  const className = [
    'team-odds-item',
    op.available ? 'between' : 'disabled',
    _.includes(orderTags, op.tag) ? 'active' : '',
    op.change,
  ];

  function OddContent() {
    if (op.locked) {
      return <DpIcon type='lockcircle' />;
    }
    if (!op.available) {
      return <span>-</span>;
    }
    return (
      <>
        <span
          className={classnames({
            'text-theme text-num': /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(
              mapBetItemName[op?.name] || op.name,
            ),
          })}>
          {(mapBetItemName[op?.name] || op.name) === op.teams.home.name
            ? '主'
            : (mapBetItemName[op?.name] || op.name) === op.teams.away.name
            ? '客'
            : (mapBetItemName[op?.name] || op.name).split(' ').map((_: string, midx: number) => (
                <span
                  className={classnames({
                    'text-theme text-num': /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(_),
                    'ml-2': midx === 1,
                  })}
                  key={midx}>
                  {_}
        ...
        <span className={classnames('odd')}>{getViewOdd(op.od, op.oddBetType)}</span>
      </>
    );
  }
