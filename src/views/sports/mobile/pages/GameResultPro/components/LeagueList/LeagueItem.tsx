...
export default React.memo((props: LeagueItemPrps) => {
  ...
  const _itemContent = (item:any[])=> {
    return (
      <>
        <div className={style.league_wrapper} onClick={()=> {
          props.toggleCollapseLeague(item[0].leagueId);
        }}>
          <div className={style.league_info}>
            <span className={style.league_name}>
              {item[0].ln}
            </span>
          </div>
          <div className={style.actions}>
            <DpIcon className={classnames(style.icon, {[style.icon_right]: props.isCollapse.includes(String(item[0].leagueId))})} width={12} height={12} type="arrow" fill={mobileTheme.dpAncillary} />
          </div>
        </div>
        <div className={style.item_content}>
          {
            item.map((data:any)=>{
              return <div className={style.item_bet} key={data.mid}>
                <div className={style.item_time}>{dayjs(data.bt).format('MM-DD hh:mm')}</div>
