    ...
    <div className={style.wrapper}>
      ...
      <div className={style.navbar}>
        {displayTypes.map((val, key) => (
          <div
            key={key}
            className={classnames(style.text_wrapper, {
              [style.active]: matchType === key,
            })}
...
export const DatePicker = React.memo((props: DatePickerProps) => {
  ...
  useEffect(() => {
    const inDateRange = dates.findIndex(
      (date) => date.time === props.currentTime
    );
    if (inDateRange > -1) {
  ...
  return (
    <div className={style.date_list}>
      {dates.map((item, idx) => (
        <div
          className={classnames(style.option_item, {
