...
import {
  useLeagueListData,
  useMatchListData,
} from "@core/hooks/sports/useRenderData";
...
export default React.memo(() => {
  ...
  const currentLeagueId = useSelector(
    (state: TStore) => state.sport.display.currentLeagueId
  );
  const displayType = useSelector(
    (state: TStore) => state.sport.display.displayType
  );
  const matchType = useSelector(
    (state: TStore) => state.sport.userSettings.matchType
  );
  const sportId = useSelector(
    (state: TStore) => state.sport.userSettings.sportId
  );
  const { orderTags, isSeries } = useOrderCart();
  const visibleBetOrderMitt = useEventEmitter<TMitt["visibleBetOrder"]>({
    mittName: "visibleBetOrder",
  });
  const {
  ...
  const cacheLeagueList: TLeagueStatistic[] = React.useMemo(
    () =>
      matchType === EMatchTypes.CHAMPION
        ? []
        : storage.getAny(`${sportId}_league_list`),
    [sportId, matchType]
  );
  const cacheMatchList: TMatch[] = React.useMemo(
    () =>
      matchType === EMatchTypes.CHAMPION
        ? []
        : storage.getAny(`${sportId}_match_list`),
    [sportId]
  );
  const [currentIndex, setCurrentIndex] = useState(0);
  useEventEmitter<TMitt["syncMethodsTab"]>({
    mittName: "syncMethodsTab",
    on: ({ index, sportId }) => {
    ...
    if (el) {
      if (
        ...
        document
          .querySelector(
            `.${
              type === EMatchTypes.IN_PLAY
                ? EMatchTypes.UPCOMING
                : EMatchTypes.IN_PLAY
            }-end`
          )
  ...
  if (
    ...
    (matchType === EMatchTypes.EARLY ||
      !cacheLeagueList?.length ||
      !cacheMatchList?.length)
  ) {
    return (
      <div className="pt-20">
        <MatchListSkeleton />
  ...
  return (
    <div className={styles.wrapper}>
      {_.map(
        _.groupBy(
          _.orderBy(
            ...
            ["state"],
            [matchType === EMatchTypes.IN_PLAY ? "asc" : "desc"]
          ),
          "state"
        ),
        (leaguesGroup: Array<TLeagueStatistic>, key: EMatchTypes) => (
          <React.Fragment key={key}>
            <div
              className={classnames("play-type-wrapper", key, {
                "is-closed":
                  key === EMatchTypes.IN_PLAY
                    ? !isCollapseInplay
                    : !isCollapseUpcoming,
              })}
            ...
            >
              <div className="name">
                ...
                <span>
                  {matchType === EMatchTypes.EARLY
                    ? "早盘"
                    : _.find(ALL_MATCH_TYPES, { code: key })?.name}
                </span>
                &nbsp;
                <span className="count">
                  ({_.sumBy(leaguesGroup, (league) => league.countGroup[key])})
                </span>
              </div>
            ...
            {leaguesGroup.map((league, index) => (
              <LeagueItem
                ...
                cacheMatches={
                  _.groupBy(cacheMatchList, "leagueId")[league.leagueId]
                }
      ...
      {false && (
        <div
          className={classnames("collapse-icon", { closed: !isCollapseAll })}
          onClick={onToggleAll}
        ></div>
      )}
    </div>
...
export const LeagueItem = React.memo((leagueItem: ILeagueItem) => {
  ...
  const { emit } = useEventEmitter<TMitt["syncMatchPollingLeagueIds"]>({
    mittName: "syncMatchPollingLeagueIds",
  });
  const sportId = useSelector(
    (state: TStore) => state.sport.userSettings.sportId
  );
  ...
  const { targetRef, visible } = useIntersectionObserver(
    (_: boolean, boundingClientRect: any) => {
  ...
  if (state) {
    matchWithState = list?.filter(
      (item) => item.isLive === (state === EMatchTypes.IN_PLAY)
    );
  }
  useUnmount(() => {
  ...
  const matchIdsInLeague = matchIds
    ? matchIds[state]
    : _.map(list, (i) => i.matchId);
  return (
    <div
      className={classnames(styles.leagueItem, leagueItem.className, {
        "in-view": inView,
      })}
      ref={targetRef}
    >
      {visible ? (
        <React.Fragment>
          <div
            className={classnames("league-wrapper", {
              "is-closed": !_.includes(
                collapseLeagueIds,
                `${statePrefix}#${leagueId}`
              ),
            })}
            onClick={() =>
              !isCache &&
              toggleCollapseLeague({
                leagueId: `${statePrefix}#${leagueId}`,
                isRequestData: !matches,
              })
            }
          >
            <div className="league-info">
              <DpIcon
                type="collect"
                active={isFavorite(matchIdsInLeague)}
                onClick={(e) =>
                  !isCache && onToggleFavorite(e, matchIdsInLeague)
                }
              />
              <span className="league-name ml-5">
                ...
                {matchType === EMatchTypes.CHAMPION && leagueItem.startTime && (
                  <span className="ml-10">
                    {dayjs(leagueItem.startTime).format("YYYY年MM月DD日 HH:mm")}
                  </span>
                )}
                {countGroup &&
                  matchType !== EMatchTypes.CHAMPION &&
                  !isCache &&
                  !_.includes(collapseLeagueIds, `${state}#${leagueId}`) && (
                    <span className="ml-10">{countGroup[state]}</span>
                  )}
              </span>
            </div>
          </div>
            ...
            <div className="match-items-wrapper" ref={ref}>
                    ...
                    <div className="name-list">
                      {
                        <div className="match-title">
                          <MatchMethodNames
                            sportId={leagueItem.sportId ?? sportId}
                            actIndex={leagueItem.actIndex}
                          />
                        </div>
                      }
                    </div>
              ...
              {matchWithState?.length > 0 &&
                matchWithState.map((item: TMatch) =>
                  item.isChampion ? (
                    <MatchChampion match={item} key={item.matchId} />
                  ) : item.sportId > 33 ||
                    gameBettingType === EGameBettingType.BEGINNER ? (
                    <MatchOfNoob match={item} key={item.matchId} />
                  ) : (
                    <MatchMethods
                      isUnfoldBownBtn={isUnfoldBownBtn}
                      setIsUnfoldBownBtn={setIsUnfoldBownBtn}
                      actIndex={leagueItem.actIndex}
                      match={item}
                      key={item.matchId}
                      inMainList={true}
              ...
              {matchWithState?.length > 0 &&
                matchWithState.map((item: TMatch) =>
                  ...
                  gameBettingType === EGameBettingType.BEGINNER ? (
                    <div
                      key={item.matchId}
                      className="match-beginner-placeholder"
                    ></div>
                  ) : (
