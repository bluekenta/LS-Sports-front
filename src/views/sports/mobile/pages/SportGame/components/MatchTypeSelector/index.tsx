...
export default React.memo(() => {
  ...
  const { emit } = useEventEmitter<TMitt["toggleFavorite"]>({
    mittName: "toggleFavorite",
    on: ({ display }) => setIsFavorite(display),
  });
  const { emit: showDateEmit } = useEventEmitter<TMitt["showDatePick"]>({
    mittName: "showDatePick",
  });
  ...
  const onChangeType = (index: number, type?: EMatchTypes) => {
    ...
    dispatch(
      ACTIONS.BASE.toggleSerierWay({ data: type === EMatchTypes.PARLAY })
    );
  };
  useEffect(() => {
  ...
  return (
    <>
      <div className={css.wrapper}>
          ...
          <div className="navbar">
            <div
              className={classnames("text-wrapper", {
                active: !isFavorite && selectIndex === 0,
              })}
              onClick={() => onChangeType(0, EMatchTypes.IN_PLAY)}
            >
              今日
            </div>
            <div
              className={classnames("text-wrapper", {
                active: !isFavorite && selectIndex === 1,
              })}
              onClick={() => onChangeType(1, EMatchTypes.IN_PLAY)}
            >
              滚球
            </div>
            <div
              className={classnames("text-wrapper", {
                active: !isFavorite && selectIndex === 2,
              })}
              onClick={() => onChangeType(2, EMatchTypes.EARLY)}
            >
              早盘
            </div>
            <div
              className={classnames("text-wrapper", {
                active: !isFavorite && selectIndex === 4,
              })}
              onClick={() => onChangeType(4, EMatchTypes.PARLAY)}
            >
              串关
            </div>
            <div
              className={classnames("text-wrapper", {
                active: !isFavorite && selectIndex === 3,
              })}
              onClick={() => onChangeType(3, EMatchTypes.CHAMPION)}
            >
              冠军
            </div>
        ...
        <div
          className={classnames("div-2", { "rotate-infinite": loadingProfile })}
          onClick={() => getProfile()}
        >
          <div className="group">
          ...
          <div className="text-wrapper-3">
            {formatCurrency(user.info.totalBalance)}
          </div>
        </div>
      </div>
    </>
...
export const DatePicker = React.memo(() => {
  const earlyGroup = useSelector(
    (state: TStore) => state.sport.display.earlyGroup
  );
  ...
  const dates = React.useMemo(() => {
    const arr = [
      { name: "全部", count: earlyGroup[0], query: 0 },
      ...week,
      { name: "其他", count: earlyGroup[8], query: 8 },
    ];
    if (matchType === EMatchTypes.PARLAY)
      arr.splice(0, 1, { name: "今日", count: earlyGroup[9], query: 9 });
    return arr;
  }, [matchType]);
  ...
  return (
    <div className="date-list">
      {dates.map((item, idx) => (
        <div
          className={classnames("option-item", { active: idx === index })}
          onClick={() => handleClick(idx, item.query)}
          key={item.name}
        >
          {item.name}
        </div>
