...
import {
  getBetHandiCapAtBetting,
  getBetTeamType,
  getPlayNameByKc,
  isChampion,
  isVisiableSecondText,
} from "@core/utils";
import { getSeriesDigitsStr } from "@core/utils/math";

export default () => {
  const { confirmOrders, removeConfirmedOrders, isSeries, checkResult } =
    useOrderCart();
  ...
  const getProductAmountTotal = () => {
    return _.reduce(
      confirmOrders,
      (result, item) => result + item.productAmountTotal,
      0
    );
  };
  const getmaxWinAmountTotal = () => {
    const maxWinAmount = _.reduce(
      confirmOrders,
      (result, item) => result + item.maxWinAmount,
      0
    );
    return maxWinAmount;
  };
    ...
    return (
      <div className={style.noticeWrapper}>
        <div className="icon-success">
          {!checkResult.some((item) => item.status !== 0) && (
            <em>注单已确认&nbsp;</em>
          )}
          {checkResult.find((item) => item.status === 3) && (
            <>
              {checkResult.filter((item) => item.status === 3).length}
              笔注单待确认&nbsp;
            </>
          )}
          {checkResult.find((item) => [2, 4, 5].includes(item.status)) && (
            <>
              {
                checkResult.filter((item) => [2, 4, 5].includes(item.status))
                  .length
              }
              笔注单未完成
            </>
              ...
              <div className="odds">
                <em>
                  ...
                  {isVisiableSecondText(
                    ...
                    <em>
                      {getBetHandiCapAtBetting(item.betHandicap, item.sportId)}
                    </em>
                  )}
                </em>
                <p>
              ...
              <div className="info-content">
                <ul className="info-list">
                  <li>
                    {getPlayNameByKc({
                      code: item.kindsCode,
                      name: item.betItemName,
                      ctid: item.betItemCTID,
                    })}
                  </li>
            ...
            <li className="series-item-info" key={item.id}>
              <div className="s-1">
                <span>
                  {getSeriesDigitsStr(
                    item.seriesType,
                    confirmOrders[0].details.length
                  )}
                </span>
              ...
              <div className="s-2">
                ...
                <div>
                  <span>投注金额</span>
                  <em className="bet-amount">
                    {formatCurrency(item.productAmountTotal || 0)}
                  </em>
                </div>
              </div>
            </li>
        ...
        <div className="score-info">
          <div className="odds">
            <em>
              ...
              <em className="space"></em>
              {isVisiableSecondText(
                confirmOrders[0].details[0].betHandicap,
                confirmOrders[0].details[0].betN,
                confirmOrders[0].details[0].kindsCode
              ) && (
                <em>
                  {getBetHandiCapAtBetting(
                    confirmOrders[0].details[0].betHandicap,
                    confirmOrders[0].details[0].sportId
                  )}
                </em>
              )}
            </em>
            <p>
              <span>@</span>
              <span>
                {formatCurrency(confirmOrders[0].details[0].oddValue)}
              </span>
            </p>
          </div>
          <div className="info-content">
            <ul className="info-list">
              <li>
                {getPlayNameByKc({
                  code: confirmOrders[0].details[0].kindsCode,
                  name: confirmOrders[0].details[0].betItemName,
                  ctid: confirmOrders[0].details[0].betItemCTID,
                })}
              </li>
