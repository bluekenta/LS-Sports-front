...
export default () => {
  ...
  const reloadEmit = useEventEmitter<TMitt["isReload"]>({
    mittName: "isReload",
  });
  const [showFavorite, setShowFavorite] = React.useState(false);
  useEventEmitter<TMitt["toggleFavorite"]>({
    mittName: "toggleFavorite",
    on: ({ display }) => setShowFavorite(display),
  });
    ...
    <div className={css.wrapper}>
      {!isESports() && (
        <div className="item" onClick={handleHandicapTutorial}>
          <img
            src={require(`./i/icon${theme === ETHEME.DARK ? "-" : "_"}1.png`)}
          />
          <span>盘口教程</span>
        </div>
      )}
      <div className="item" onClick={() => handlePopUp(SportPopup.SETTING)}>
        <img
          src={require(`./i/icon${theme === ETHEME.DARK ? "-" : "_"}2.png`)}
        />
        <span>设置菜单</span>
      </div>
      <div className="item" onClick={() => toggleMobileOrderHistory(2, 1)}>
        <img
          src={require(`./i/icon${theme === ETHEME.DARK ? "-" : "_"}3.png`)}
        />
        <span>已结算注单</span>
      </div>
      <div className="item" onClick={() => toggleMobileOrderHistory(2, 0)}>
        <img
          src={require(`./i/icon${theme === ETHEME.DARK ? "-" : "_"}4.png`)}
        />
        <span>未结算注单</span>
      </div>
      <div className="item" onClick={handleReload}>
        <div className="icon-wrap">
          <img
            className={`${isReload && "rotate-infinite"}`}
            src={require(`./i/icon${theme === ETHEME.DARK ? "-" : "_"}5.png`)}
          />
        </div>
        <span>刷新</span>
      </div>
