...
export default ({
  ...
  const { matchListUpdateTime } = useSelector(
    (state: IStore) => state.sport.display
  );
  ...
  const isUnfold = React.useMemo(() => {
    return Object.values(toggleBtn).some(
      (tggle: any) => tggle.toggle && _.find(playMethod, { ctid: tggle.ctid })
    );
    ...
    <div className={style.wrapper}>
      {
        <>
          <div className="phase" onClick={handleOpenDetail}>
            <div className="flex items-center">
              ...
              {match.matchClock.isRunning ? (
                <>
                  ...
                  {match.matchClock.period !== "HT" &&
                  match.matchClock.period !== "FT" &&
                  match.matchClock.second > 0 &&
                  match.sportId === 1 ? (
                    <CountdownLite
                      seconds={match.matchClock.second}
                      key={`${match?.matchId}_${match.matchClock.second}_${matchListUpdateTime}`}
                    />
                  ) : (
            ...
            <span className="fs-10 flex items-center">
              ...
              <DpIcon
                ...
                width={9}
                height={10}
                fill={mobileTheme.dpNormal}
              />
            </span>
          </div>
          <div className="match-info">
            <div className="infos" onClick={handleOpenDetail}>
              <div className="team-list">
                <div className="team">
                  <div className="team-name">
                    <span>{match.teams.home.name}</span>
                    {match.score.homeRedCard > 0 && (
                ...
                <div className="media-list">
                  ...
                  }
                  {
                    <DpIcon
                      className="mr-4"
                      type="animate"
                      active={match.hasAnimate}
                    />
                  }
                  {match.hasCorner && (
                    <img
                      style={{ width: "18px" }}
                      src={require("@this/assets/images/common/ico_cornerkick.png")}
                    />
                  )}
                </div>
          ...
          {match.sportId === 1 && playMethod.length > 0 && (
            <div className={classnames("match-info", "match-info-sub")}>
              <div className="infos">
                <div className="btn-list">
                  {playMethod.map((item: TPlayMethodItem) => (
                    <div
                      key={item.type}
                      className={classnames("btn", {
                        active: toggleBtn[item.type as toggleBtnType].toggle,
                      })}
                      onClick={() =>
                        handleToggleBtn(item.type as toggleBtnType)
                      }
                    >
                      <span>{item.name}</span>
                      <DpIcon
                        type="arrow"
                        fill={
                          toggleBtn[item.type].toggle
                            ? mobileTheme.dpTheme
                            : mobileTheme.dpAncillary
                        }
                        className={classnames(
                          `arrow ${
                            toggleBtn[item.type as toggleBtnType].toggle
                              ? "open-all-matchs"
                              : "close-all-matchs"
                          }`
                        )}
                ...
                {isUnfold && (
                  <div className="teamName">
                    <div
                      className={classnames("team-name flex")}
                      onClick={handleOpenDetail}
                    >
                      <Tooltip title={match.teams.home.name} placement="top">
                        {match.teams.home.name}
                      </Tooltip>
                      {subCtid === 1 && match.isLive && (
                        <span>{match.score.homeCorner}</span>
                      )}
                    </div>
                    <div
                      className={classnames("team-name flex")}
              ...
              {isUnfold && (
                <div className="odds">
                  <SubOdds
                    ...
                    mks={match.playTypes}
                    ctid={subCtid}
                    isbodan={toggleBtn.bodan.toggle}
                  />
                </div>
              )}
            </div>
