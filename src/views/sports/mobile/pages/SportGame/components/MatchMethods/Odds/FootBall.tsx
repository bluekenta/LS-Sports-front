...
export default ({ data, actIndex, isbodan = false }: IProps) => {
  const currentOddType = useSelector(
    (state: TStore) => state.user.currentOddType
  );
  const playTypes = getPlayTypesBySportId(ESportsCategory.FOOTBALL);
  const swiperRef = React.useRef(null);

  const { emit } = useEventEmitter<TMitt["syncMethodsTab"]>({
      ...
      >
        {!isbodan
          ...
          : data.map((play, indx) => (
              <SwiperSlide
                key={indx}
                className={classnames(css.oddsWrapper, "bodan-swiper")}
              >
                <div className="bd-list bodan-list">
                  <div className="game-name">
                    {play?.mks[0]?.ops[0]?.teams.home.name}
                  </div>
                  <div className="game-name">
                    {play?.code === "FT_CS" ? "全场平局" : "半场平局"}
                  </div>
                  <div className="game-name">
                    {play?.mks[0]?.ops[0]?.teams.away.name}
                  </div>
                </div>
                <div className="bd-list">
                  {transferBoDan(play?.mks[0].ops || []).map((bil: any, idx) =>
                    bil && !bil.temporaryValue ? (
                      <OddItem
                        key={bil.id}
                        op={bil}
                        oddType={currentOddType}
                        methodCode={play.code}
                      />
                    ) : (
                      <EmptyOddItem key={idx} />
                    )
                  )}
                </div>
              </SwiperSlide>
