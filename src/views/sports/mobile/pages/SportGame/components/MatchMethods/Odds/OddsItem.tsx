...
export const LockItem = ({ col = 3 }: { col?: number }) => {
  return (
    <>
      {new Array(col).fill(0).map((_, index: number) => (
        <div key={index} className={classnames(style.item, "disabled")}>
          -
        </div>
      ))}
    </>
...
export const OddItem = ({
  ...
  return (
    <>
      <div
        className={classnames(style.item, "odd-item", {
          "is-home": isHome === 0,
          "is-draw": len === 3 && isHome === 1,
          active: _.includes(orderTags, op.tag),
        })}
        onClick={() => addAndRemoveOrder(op)}
      >
        <div className={classnames("name")}>
          <span
            className={classnames({
              "text-theme":
                !methodCode.endsWith("_AH") &&
                /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(
          ...
          >
            {!isInList &&
              methodCode.endsWith("_AH") &&
              (op.betTeam?.toUpperCase() === "HOME"
                ? op.teams.home.name
                : op.teams.away.name)}{" "}
            {getNameByhoa(op.name, methodCode, op, inMainList)
              .split(" ")
              .map((_: string, midx: number) => (
                <span
                  className={classnames({
                    "text-theme":
                      /^[+-]?[\d]+(?:\.\d+)?(?:[+\-*/]\d+(?:\.\d+)?)?$/.test(_),
